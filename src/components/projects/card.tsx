...
import { months } from '../journals/card';

export default function ProjectCard({ project }: { project: ProjectT }) {
  const [viewDesc, setViewDesc] = useState(false);
  const completed_at = new Date(project.completed_at);
    ...
    return (
      <div className=" col-span-3 bg-brand-150 rounded-3xl">
        ...
        <div className="w-full MADEMellow font-normal grid grid-cols-7 px-5 py-5">
          <div className="h-full col-span-7 mt-5 text-brand-350 MADEMellow font-light mb-3 text-xl lg:text-3xl">
            <p>
              {completed_at.getDate()} {months[completed_at.getMonth()]}{' '}
              {completed_at.getFullYear()}
            </p>
          </div>
          <span
  ...
  return (
    <div className=" col-span-3 bg-brand-150 rounded-3xl">
      <div className="w-full MADEMellow font-normal grid grid-cols-7 px-7 py-12">
        <div className="h-full col-span-7 mt-5 text-brand-350 MADEMellow font-light mb-3 text-xl lg:text-3xl">
          <p>
            {completed_at.getDate()} {months[completed_at.getMonth()]}{' '}
            {completed_at.getFullYear()}
        ...
        <div className="h-full col-span-7 mt-5 text-brand-350">
          <p>{project.desc}</p>
        </div>
        <div className="col-span-7 flex justify-end mt-12">
          <div
            // onClick={() => {
            //   setClickedInfo(!clickedInfo);
            // }}
            className="w-full md:h-full flex justify-end items-end relative cursor-pointer group"
          >
            <Link
              href={`/project/${project.id}`}
              className={` text-brand-300 text-3xl flex justify-center items-center duration-300`}
            >
